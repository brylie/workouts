...
describe('getExercisesByEquipment', () => {
	it('should return exercises that use the specified equipment', () => {
		...
		expect(exercises.every((exercise) => exercise.equipment?.includes(Equipment.TREADMILL))).toBe(
			true,
		);
	});
...
describe('filterExercises', () => {
	...
	it('should filter exercises by equipment', () => {
		...
		const exercises = filterExercises(filters);
		expect(exercises.length).toBeGreaterThan(0);
		expect(exercises.every((exercise) => exercise.equipment?.includes(Equipment.DUMBBELLS))).toBe(
			true,
		);
	});
