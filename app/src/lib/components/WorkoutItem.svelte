  ...
  <div class="mb-4 space-y-4">
    {#if item.exercise.metrics.hasSets}
      <div class="grid grid-cols-2 gap-4">
        <div>
          <label for="sets-{index}" class="mb-2 block text-sm font-medium"
            >Sets</label
          >
          <input
            type="number"
        ...
        {#if item.exercise.metrics.hasReps}
          <div>
            <label for="reps-{index}" class="mb-2 block text-sm font-medium"
              >Reps</label
            >
            <input
              type="number"
    ...
    {#if item.exercise.metrics.hasWeight}
      <div class="grid grid-cols-2 gap-4">
        <div>
          <label for="weight-{index}" class="mb-2 block text-sm font-medium"
            >Weight (kg)</label
          >
          <input
            type="number"
    ...
    {#if item.exercise.metrics.hasTime}
      <div>
        <label for="time-{index}" class="mb-2 block text-sm font-medium"
          >Time (minutes)</label
        >
        <input
          type="numeric"
  ...
  <div class="mb-4">
    ...
    <div class="flex flex-wrap gap-2">
      {#each item.exercise.muscles as muscle}
        <span class="exercise-muscle rounded bg-blue-600 px-2 py-1 text-xs"
          >{muscle}</span
        >
      {/each}
    </div>
    ...
    <div class="mb-4">
      ...
      <div class="flex flex-wrap gap-2">
        {#each item.exercise.equipment as equipment}
          <span
            class="exercise-equipment rounded bg-purple-600 px-2 py-1 text-xs"
            >{equipment}</span
          >
        {/each}
      </div>
