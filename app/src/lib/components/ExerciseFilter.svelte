<script lang="ts">
  ...
  const muscleOptions = $derived(
    [...musclesList].sort((a, b) => a.name.localeCompare(b.name)),
  );
  ...
  const muscleLabelText = $derived(
    activeMuscleCount === 1 ? "muscle" : "muscles",
  );
...
<div
  id="exercise-filter-collapse"
  class="collapse-arrow collapse rounded-lg bg-gray-800"
>
  <input type="checkbox" />
  <div class="collapse-title flex items-center gap-3 text-lg font-medium">
    ...
    {#if activeMuscleCount > 0}
      <div
        id="muscle-count-badge"
        class="badge badge-md border-blue-600 bg-blue-600"
      >
        {muscleBadgeText}
      </div>
    {/if}
    {#if activeEquipmentCount > 0}
      <div
        id="equipment-count-badge"
        class="badge badge-md border-purple-600 bg-purple-600"
      >
        {equipmentBadgeText}
      </div>
    {/if}
      ...
      <div class="flex flex-wrap gap-2">
        {#each muscleOptions as muscle}
          <label
            ...
            id="muscle-filter-{muscle.id}-label"
          >
            <input
              type="checkbox"
              class="hidden"
              ...
              onchange={(e) =>
                handleMuscleChange(muscle.id, e.currentTarget.checked)}
            />
            <span
    ...
    <div>
      ...
      <div class="flex flex-wrap gap-2">
        {#each equipmentOptions as equipment}
          <label
            class="inline-flex items-center"
            id="equipment-filter-{equipment.id}-label"
          >
            <input
              type="checkbox"
              class="hidden"
              ...
              onchange={(e) =>
                handleEquipmentChange(equipment.id, e.currentTarget.checked)}
            />
            <span
