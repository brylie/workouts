    ...
    <div class="grid grid-cols-1 gap-6 md:grid-cols-2 lg:grid-cols-3">
      {#each sortedExercises as exercise}
        <div class="exercise-card rounded-lg bg-gray-800 p-6">
          <h2 class="exercise-title mb-2 text-2xl font-semibold">
            {exercise.title}
          </h2>
          <p class="exercise-description mb-4 text-gray-300">
          ...
          <div class="mb-4">
            ...
            <div class="flex flex-wrap gap-2">
              {#each exercise.muscles as muscle}
                <span
                  class="exercise-muscle rounded bg-blue-600 px-2 py-1 text-sm"
                  >{muscle}</span
                >
              {/each}
            </div>
            ...
            <div>
              ...
              <div class="flex flex-wrap gap-2">
                {#each exercise.equipment as equipment}
                  <span
                    class="exercise-equipment rounded bg-purple-600 px-2 py-1 text-sm"
                    >{equipment}</span
                  >
                {/each}
              </div>
