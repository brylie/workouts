...
test.describe("Exercise Library Page", () => {
  ...
  test("should display exercises and filter by muscle groups", async ({
    page,
  }) => {
  ...
  test("should apply multiple filters simultaneously", async ({ page }) => {
    ...
    // Check for biceps
    const muscleLabels = await firstExercise
    ...
    const hasEquipmentSection = await firstExercise
      ...
      .isVisible();
    if (hasEquipmentSection) {
      const equipmentLabels = await firstExercise
        .locator(".exercise-equipment")
        .allInnerTexts();
      const hasDumbbells = equipmentLabels.some((label) =>
