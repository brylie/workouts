...
test.describe("Generate Workout Page", () => {
  ...
  async function expandFilterPanel(page) {
    ...
    await page
      .locator('#exercise-filter-collapse > input[type="checkbox"]')
      .check();
  ...
  test("should generate workout with filtered exercises", async ({ page }) => {
    ...
    await expect(
      page.getByRole("heading", { name: "Workout Generator" }),
    ).toBeVisible({
    ...
    for (let i = 0; i < 2; i++) {
      ...
      const muscleLabels = await card
        .locator(".exercise-muscle")
        .allInnerTexts();
      const hasChest = muscleLabels.some((label) =>
        label.toLowerCase().includes("chest"),
      );
  ...
  test("should generate workout with equipment filter", async ({ page }) => {
    ...
    const equipmentLabels = await firstCard
      .locator(".exercise-equipment")
      .allInnerTexts();
  ...
  test("should apply multiple filters to workout generation", async ({
    page,
  }) => {
    // Navigate to workout page
    await page.goto("/workouts/workout");
    ...
    const muscleLabels = await workoutCard
      .locator(".exercise-muscle")
      .allInnerTexts();
    const equipmentLabels = await workoutCard
      .locator(".exercise-equipment")
      .allInnerTexts();

    const hasBiceps = muscleLabels.some((label) =>
      label.toLowerCase().includes("biceps"),
    );
    const hasDumbbells = equipmentLabels.some((label) =>
      label.toLowerCase().includes("dumbbells"),
    );
  ...
  test("should clear filters by clicking on already selected filters", async ({
    page,
  }) => {
    // Navigate to workout page
    await page.goto("/workouts/workout");
    ...
    await expect(
      page.locator(`#muscle-filter-${Muscles.CHEST}-text`),
    ...
    await expect(
      page.locator(`#muscle-filter-${Muscles.CHEST}-text`),
    ).not.toHaveClass(/bg-blue-600/, { timeout: navigationTimeout });
    ...
    await expect(page.locator(".workout-item")).toHaveCount(3, {
      timeout: navigationTimeout,
    });
  });
});
