...
test.describe('Generate Workout Page', () => {
	...
	test('should clear filters by clicking on already selected filters', async ({ page }) => {
		...
		await expect(page.locator(`#muscle-filter-${Muscles.CHEST}-text`)).toHaveClass(/bg-blue-600/, {
			timeout: navigationTimeout,
		});
		...
		await expect(page.locator(`#muscle-filter-${Muscles.CHEST}-text`)).not.toHaveClass(
			/bg-blue-600/,
			{ timeout: navigationTimeout },
		);
