...
test.describe('Workout Generator and History Flow', () => {
  ...
  test('should generate workout and save completed exercise to history', async ({ page }) => {
    ...
    await expect(page.locator('h1')).toHaveText('Workout Generator', { timeout: navigationTimeout });
    ...
    await expect(firstExerciseCard.locator('button:has-text("Completed")')).toBeVisible({ timeout: navigationTimeout });
    
    // Navigate to history page
    await page.goto('/workouts/history');
    
    // More specific selector for the history page container
    await expect(page.getByRole('heading', { name: 'Exercise History' })).toBeVisible({ timeout: navigationTimeout });
    await expect(page.locator('h1')).toBeVisible({ timeout: navigationTimeout });
