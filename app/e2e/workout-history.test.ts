...
test.describe("Workout Generator and History Flow", () => {
  ...
  test("should generate workout and save completed exercise to history", async ({
    page,
  }) => {
    ...
    const completeButton = firstExerciseCard.locator(
    ...
    await expect(
      firstExerciseCard.locator('button:has-text("Completed")'),
    ).toBeVisible({
      timeout: navigationTimeout,
    });
    ...
    await expect(page.locator("h1")).toHaveText("Exercise History", {
      timeout: navigationTimeout,
    });
  ...
  test("should show empty state when no history exists", async ({ page }) => {
    ...
    await expect(
      page.locator(
        'p:has-text("No exercise history found for the selected date range.")',
      ),
    ).toBeVisible({ timeout: navigationTimeout });
  });

  test("workout generator settings should affect generated workout", async ({
    page,
  }) => {
    await page.goto("/workouts/workout");
